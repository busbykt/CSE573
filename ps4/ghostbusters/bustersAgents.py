...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            ...
            ghostdistances[ghostPos] = self.distancer.getDistance(pacmanPosition, ghostPos)

        closestPos = min(ghostdistances, key=ghostdistances.get)

        actiondist={}
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            actiondist[action] = self.distancer.getDistance(successorPosition, closestPos)

        return min(actiondist, key=actiondist.get)
